...
export default function PosterCard({ poster, rank = 12, title = "Anime" }) {
    ...
    return (
        /* 
          ✅ PERBAIKAN 1: Menurunkan batas lebar maksimal poster dari max-w-[240px] menjadi max-w-[150px] 
          khusus pada ponsel pintar terkecil agar tampilannya kompak, padat, dan proporsional.
        */
        <div className="relative group cursor-pointer w-full max-w-[150px] xs:max-w-[200px] sm:max-w-[260px] mx-auto select-none">
            ...
            <div
                className={`absolute -inset-1.5 sm:-inset-3 rounded-[1.2rem] sm:rounded-[2rem] blur-md sm:blur-2xl opacity-30 sm:opacity-40 group-hover:opacity-60 sm:group-hover:opacity-70 transition-all duration-700 sm:duration-1000 ${isDark
                        ? "bg-gradient-to-br from-[#ff1e56]/30 via-red-900/10 to-transparent"
                        : "bg-gradient-to-br from-rose-400/20 via-rose-200/10 to-transparent"
                    }`}
            />
            ...
            <div
                className={`absolute -inset-[1.5px] sm:-inset-[2px] rounded-[1.05rem] sm:rounded-[1.3rem] opacity-70 sm:opacity-80 group-hover:opacity-100 transition-opacity duration-500 animate-pulse-slow ${isDark
                        ? "bg-gradient-to-br from-[#ff1e56] via-red-600 to-[#8b0a1e]"
            ...
            <div
                className={`absolute -inset-[0.8px] sm:-inset-[1px] rounded-[1rem] sm:rounded-[1.25rem] ${isDark
                        ? "bg-gradient-to-br from-[#ff1e56]/40 via-transparent to-[#ff1e56]/20"
                        : "bg-gradient-to-br from-rose-300/40 via-transparent to-rose-300/20"
                    }`}
            />
            ...
            >
                ...
                <div
                    className={`absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-700 ${isDark
                            ? "bg-gradient-to-br from-[#ff1e56]/5 via-transparent to-transparent"
                            : "bg-gradient-to-br from-rose-400/5 via-transparent to-transparent"
                        }`}
                />
                ...
                {/* Scanlines — dark only */}
                {isDark && (
                    <div className="absolute inset-0 opacity-[0.03] pointer-events-none bg-[linear-gradient(transparent_50%,rgba(0,0,0,0.25)_50%)] bg-[length:100%_4px]" />
                )}
                ...
                <div className="absolute top-1.5 left-1.5 sm:top-3 sm:left-3 z-10">
                    <div className="relative">
                        ...
                        />
                        <div className="relative flex items-center gap-1 bg-gradient-to-r from-[#ff1e56] to-[#e11d48] text-[#ffffff] text-[7.5px] sm:text-[10px] font-black px-1.5 py-0.5 sm:px-2.5 sm:py-1 rounded-md sm:rounded-lg tracking-wider uppercase shadow-lg">
                ...
                <div className="absolute bottom-0 left-0 right-0 p-2.5 sm:p-4 z-10">
                    ...
                    {/* Decorative line */}
                    <div
                        className={`h-px bg-gradient-to-r mb-1.5 sm:mb-2 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity duration-700 delay-100 ${isDark
            ...
            <div
                className={`absolute -bottom-2 sm:-bottom-4 left-3 sm:left-4 right-3 sm:right-4 h-4 sm:h-8 blur-md sm:blur-xl opacity-0 group-hover:opacity-100 transition-opacity duration-700 ${isDark
                        ? "bg-gradient-to-t from-[#ff1e56]/10 to-transparent"
                        : "bg-gradient-to-t from-rose-400/10 to-transparent"
                    }`}
            />
        </div>
    );
