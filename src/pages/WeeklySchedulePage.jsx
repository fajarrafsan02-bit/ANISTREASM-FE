...
// Hari default berdasarkan hari ini
        ...
        <div className={`min-h-screen transition-colors duration-500 py-4 sm:py-8 relative overflow-hidden ${isDark ? 'bg-[#050203]' : 'bg-[#faf8f5]'}`}>
            ...
            <main className="w-full max-w-[1440px] mx-auto px-3 sm:px-4 md:px-8 flex flex-col gap-5 sm:gap-8 relative z-10">
                ...
                <header className="relative">
                    <div className={`absolute top-0 left-0 right-0 h-px bg-gradient-to-r from-transparent ${isDark ? 'via-[#ff1e56]/20' : 'via-slate-300'} to-transparent`} />
                    <div className="pt-5 pb-4 border-b border-dashed relative">
                        <div className={`absolute bottom-[-1px] left-0 w-3 h-3 border-l-2 border-b-2 ${isDark ? 'border-[#ff1e56]/30' : 'border-slate-300'}`} />
                        <div className={`absolute bottom-[-1px] right-0 w-3 h-3 border-r-2 border-b-2 ${isDark ? 'border-[#ff1e56]/30' : 'border-slate-300'}`} />
                        <div className="flex items-center gap-2.5 sm:gap-3 mb-1">
                            <div className={`w-1 h-7 sm:h-8 md:h-10 rounded-full bg-gradient-to-b ${isDark ? 'from-[#ff1e56] to-rose-800' : 'from-rose-500 to-rose-700'}`} />
